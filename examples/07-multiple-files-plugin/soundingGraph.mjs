...
const init = _refs => {
	...
	if(rs.isMobile || rs.isTablet) {
		chartArea.on('touchstart', dragStarted)
			.on('touchmove', dragged)
	} else {
		chartArea.call(d3.drag()
	...
	const infoLine = chartArea.append('g')
		.attr('class', 'infoLine')
	...
	const labels = infoLine.append('g')
		.attr('transform', `translate(0,-5)`)

	labels.append('text').attr('class', 'alt')
		.attr('x', 10)

	const tempLabels = labels.append('g')
		.attr('transform', `translate(${chartWidth/2},0)`)
		.attr('text-anchor', 'end')

	tempLabels.append('text').attr('class', 'dewpoint')
		.attr('x', 20)
		.attr('text-anchor', 'end')

	tempLabels.append('text').attr('class', 'temp')
		.attr('x', 23)
		.attr('text-anchor', 'start')

	labels.append('text').attr('class', 'windSpeed')
		.attr('x', chartWidth - 18)
		.attr('text-anchor', 'end')

	labels.append('text').attr('class', 'windDir')
...
function calcCurrentInfo() {
	...
	currentInfo.lcl = sUtils.intersection(humidityLine, [[p1.temp, p1.gh],
		[p1.temp - 0.01 * (p2.gh - p1.gh), p2.gh]]) // 1°C / 100m
}
...
function updateInfoLine() {
	...
	infoLine.select('.alt').text(
		...
		+ (pt.level ? ` ${convertPressure(Math.round(pt.level) * 100)}${overlays.pressure.metric}` : '')
	)
	infoLine.select('.temp').text(`${convertTemp(pt.temp.toFixed(1))}${overlays.temp.metric}`)
	infoLine.select('.dewpoint').text(`${convertTemp(pt.dewpoint.toFixed(1))}${overlays.temp.metric}`)
	infoLine.select('.windSpeed').text(`${convertWind(wind.wind.toFixed(1))}${overlays.wind.metric}`)
...
function updateInfoBox() {
	...
	refs.alt.textContent = `altitude: ${convertAlt(pointData.elevation)}${overlays.cloudtop.metric}`
	refs.modelAlt.textContent = `model altitude: ${convertAlt(pointData.modelElevation)}${overlays.cloudtop.metric}`
	refs.modelAlt.className = Math.abs(pointData.modelElevation - pointData.elevation) > 200 ? 'red' : 'ok'
}
...
const setXScale = () => {
	...
	xAxisScale.domain([convertTemp(range[0]),
		convertTemp(range[1])])
...
const updateSettings = () => {
	...
	svg.select('.x.label')
		.text(overlays.temp.metric)

	svg.select('.y.label')
		.text(`${overlays.cloudtop.metric}/amsl`)
...
const redraw = () => {
	...
	currentData = sUtils.interpolateArray(pointData.data[ts1], pointData.data[ts2],

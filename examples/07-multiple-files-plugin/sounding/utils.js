...
export const almostEqual = (a, b) => Math.abs(a - b) < 0.000001;

// Bilinear interpolation between two data arrays
export function interpolateArray(data1, data2, w) {
    if (almostEqual(w, 0)) {
        return clone(data1);
    }
    if (almostEqual(w, 1)) {
    ...
    for (let lev = 0; lev < data1.length; ++lev) {
        ...
        const d2 = lev == 0 ? data2[0] : data2.find(d => d.level == data1[lev].level);
        if (!d2) {
            continue;
        }
...
export function interpolatePoint(point1, point2, w) {
    if (almostEqual(w, 0)) {
        return clone(point1);
    }
    if (almostEqual(w, 1)) {
...
export function intersection(line1, line2) {
    ...
    const d = (y2 - y1) * (x4 - x3) - (x2 - x1) * (y4 - y3);
    if (!d) {
        return null;
    } // Lines are parallel

    const t = ((x2 - x1) * (y3 - y1) + (y2 - y1) * (x1 - x3)) / d;
    if (t < 0 || t > 1) {
        return null;
    ...
    const s = x2 - x1 ? (px - x1) / (x2 - x1) : (py - y1) / (y2 - y1);
    if (s < 0 || s > 1) {
        return null;
...
export function dataIntersection(line, polyline, getPoint) {
...
export function addWindBarb(g, x, y, dir, speed) {
    const scale = 20;
    const windBarb = g
        .append('g')
        .attr('class', 'windbarb')
